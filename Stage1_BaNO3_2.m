function [Lambda,Lambda_dt,lambda_SO4_t0]=Stage1_BaNO3_2(bulk,SBNCAN,V,upsilon,t)
...
%第1项  代表SO4
    a1=V * SBNCAN.C0(1) - upsilon .* SBNCAN.C0(2).* t;
    a1_dt=-upsilon .* SBNCAN.C0(2);

    Gamma=   Gamma    + 0.5 *  SBNCAN.z(1)^2 .*a1 ./ b;
    Gamma_dt=Gamma_dt + 0.5 *  SBNCAN.z(1)^2 .*(a1_dt.*b - a1.*b_dt) ./ b.^2;

%第2项 代表Ba没有

%第3项 代表来自Ba(NO3)2的NO3
    a3=upsilon .* SBNCAN.C0(3) .* t;
    a3_dt=upsilon .* SBNCAN.C0(3);
...
lambda_SO4_t0=lambda_SBNCAN(i,1);
...
%第1项 SO4的贡献
i=i+1;
A1=lambda_SBNCAN(i,:) .* a1;
A1_dt=lambda_SBNCAN_dt(i,:) .* a1 + lambda_SBNCAN(i,:).*a1_dt;

Lambda=Lambda + SBNCAN.z(1) .*  A1 ./ b;
Lambda_dt=Lambda_dt + SBNCAN.z(1) .* (A1_dt.*b - A1.*b_dt) ./ b.^2;
