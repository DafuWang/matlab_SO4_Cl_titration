function [Lambda,Lambda_dt]=Stage4_AgNO3(bulk,SBNCAN,V,upsilon,t1,t2,t3,t)
...
% 第5项 代表Ag
    ...
    a5=upsilon .* (t-t3) .* SBNCAN.C0(5);
    a5_dt=upsilon .* SBNCAN.C0(5);

    Gamma=   Gamma    + 0.5 *SBNCAN.z(4)^2. * a5 ./ b;
    Gamma_dt=Gamma_dt + 0.5 * SBNCAN.z(4)^2. * (a5_dt.*b - a5.*b_dt) ./ b.^2;
...
% 第5项没有
i=i+1;
A5=lambda_SBNCAN(i,:) .* a5;
A5_dt=lambda_SBNCAN_dt(i,:).*a5 + lambda_SBNCAN(i,:).*a5_dt;

Lambda=Lambda + SBNCAN.z(j) .* A5 ./ b;
Lambda_dt=Lambda_dt + SBNCAN.z(j) .* (A5_dt.*b - A5.*b_dt) ./ b.^2;
